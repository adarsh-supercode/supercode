...
import * as styles from "../css/resourcesDetails.module.css";
...
export default function ResourcesDetails({ data }) {
  const [url, setUrl] = useState("");
  const { title, content, date, categoryNames, acf } = data || {};
  ...
  return (
    <div className="noiseMainWrap">
      <div className={styles.singlePostSection}>
        <div className="container">
          <h1 className="heading-3-light">
            {title?.rendered || "Untitled Post"}
          </h1>
        </div>
        <div className={styles.singlePostDetails}>
          <div className={styles?.singlePostsideNav}>
            <div>
              <p className="text-5">{acf.read_time}</p>
            </div>
            <div className={styles?.dateDiv}>
              <p className="text-5">Posted on</p>
              <p className={`${styles.postDate} text-5`}>{formattedDate}</p>
            </div>
            <div className={styles?.shareLinkDiv}>
              <p className={`${styles?.shareLinkTitle} text-5`}>
                Share article
              </p>
              <SocialShare
                dataToShare={url}
                className={styles?.SocialShareIcons}
              />
            </div>
            <div className={styles?.singlePostCategories}>
              {categoryNames?.map((e, index) => {
          ...
          <div
            dangerouslySetInnerHTML={{
              __html: content?.rendered || "<p>No content available</p>",
            }}
        ...
        <div className="container">
          <div className={styles?.formsWrap}>
            <div>
              <p className={`${styles?.postMailDetails} text-4-reg`}>
                Subscribe to our articles.
              </p>
              <form
                className={`${styles?.singleinsightForm}`}
              ...
              >
                ...
                />
                <PrimaryButton type={"submit"} label="Submit" noBg={true} />
