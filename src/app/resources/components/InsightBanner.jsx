...
export default function InsightBanner({ banner }) {
  const { title, heading, description, formtext } = banner || {};
  ...
  return (
    <div className="container">
      <div style={{ paddingTop: "160px" }}>
        <p className="text-6-med">{title}</p>
        <div className={styles?.bannerContentWrap}>
          <div
            dangerouslySetInnerHTML={{ __html: heading }}
            className={`${styles?.InsightBannerHeading} heading-3`}
          />
          <div className={styles?.formSec}>
            <p className={`${styles?.InsightBannerDesc} text-3-reg `}>
              {description}
            </p>
            <form className={`${styles?.insightForm}`} onSubmit={handleSubmit}>
              <input
                type="email"
                name="userEmail"
              ...
              />
              <PrimaryButton type={"submit"} label="Submit" noBg={true}  className="text-5"/>
