...
import * as styles from "../privacy/privacy.module.css";
import { generateSEO } from "@/utilites/helper";

export async function generateMetadata() {
  let data = await fetch(`${process.env.SERVER_PAGE_URL}2017?acf_format=standard`, {
    next: { revalidate: 200 },
  });
...
export default async function Page() {
  try {
    const response = await fetch(
      `${process.env.SERVER_PAGE_URL}2017?acf_format=standard`,
      {
        next: { revalidate: 60 },
      }
    );
