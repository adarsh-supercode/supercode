...
export async function generateMetadata() {
  let data = await fetch(`${process.env.SERVER_PAGE_URL}1736?acf_format=standard`, {
    next: { revalidate: 200 },
  });
...
export default async function page() {
  try {
    const response = await fetch(
      `${process.env.SERVER_PAGE_URL}1736?acf_format=standard`,
      {
        next: { revalidate: 60 },
      }
    ).then((res) => res.json());
  ...
  return (
    <div>
      <WebdesignPage pageData={pageData}/>
    </div>
  );
} else {
